...
###############################ALTERNATIVE _DATA _STRUCTURE
'''
    Instead of using sets like the other script ("orthologs_vc49_1.py" to  find the intersection. Here, I use the hybrid version of set and list which technically
    should reduce the run time only  big O(n) where n is the length of the shortest list/set.
...
if len(sys.argv)==4 or sys.argv[4].upper()=='TRUE': #boolean flag is optional or if true, it would return a reciprocal ortholog match
    set_xml_2= set(xml_2_key_value_in_a_list)  #converting a list to a set
    intersection = [orthologs for orthologs in xml_1_key_value_in_a_list if orthologs in set_xml_2] #returning values  if intersect between a list and a set
    ...
    union = xml_1_key_value_in_a_list+xml_2_key_value_in_a_list #finding union of the two  lists by extending the list...Duplicate pairs could be there but could be easily removed later using linux command 
    for l in union:  #writing out the output file delimited by tab and separated by two indentations in between each pair
        output.write('%s\n\n' % l )
    
#####This script runs perfectly on the terminal. The only issue I have here is I feel like there must be a way to use dictionary directly to find the intersect which would decrease the algorithm complexity drastically. However, I run into issue of duplicate dictionary keys' collisions! Let me know if you know of the simplest way possible using a dictionary data structure directly.